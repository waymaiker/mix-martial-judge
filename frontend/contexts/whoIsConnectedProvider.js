...
export const WhoIsConnectedProvider = ({ children }) => {
  ...
  const getAdmins = async () => {
    ...
    const contractEvent = new ethers.Contract(process.env.NEXT_PUBLIC_FIGHT_SCADDRESS_GOERLI, FightContract.abi, provider)
    const dataEvents = await contractEvent.queryFilter({ address: process.env.NEXT_PUBLIC_FIGHT_SCADDRESS_GOERLI, fromBlock: 0 })
