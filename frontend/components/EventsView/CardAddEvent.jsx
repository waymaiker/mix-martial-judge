...
import { eventContract } from '@/utils/constants';
...
export default function CardAddEvent() {
  ...
  const submit = async () => {
    setIsLoading(true);
    try {    
      const contract = new ethers.Contract(eventContract.address, eventContract.abi, signer);
      const transaction = await contract.create(fighterOne, fighterTwo, arena, location, fightType, fileData.link, fileData.CID);
  ...
  return (
    <Flex direction={"column"} alignItems="center" mt="5">
      ...
      <Text fontWeight="bold" fontSize="7xl" fontStyle="italic">  CREATE AN EVENT</Text>
